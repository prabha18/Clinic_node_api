    ...
    try {
        if (req.jwtToken) {
            ...
            if (decoded) {
                ...
                if (city_id == 0) {
                    await client.query(`INSERT INTO "tbl_def_city"("city_id","city_name","active_status") values ($1, $2,$3) `, [maxCity_Check, city_name, 1]);
                }
                ...
                if (account_id == 0) {
                    ...
                    if (moduleArray && moduleArray.length > 0) {
                        for (let i = 0; i < moduleArray.length; i++) {
                            ...
                            console.log(accountmodule_code)
                        }
                    }
                    if (client) {
                        client.end();
                    ...
                    if (count_Check != 0 && count_Check != null && count_Check != undefined && count_Check != "") {
                        const update_result = await client.query(`UPDATE "tbl_account_master" set "company_name"= Upper($1), "city_id"=$2,"state_id"=$3,"company_code"=$4,"email_id"=$5,"area_name"=$6,"street_name"=$7,"gstin_number"=$8,
...
module.exports.listAccount = async (req) => {
    ...
    try {
        if (req.jwtToken) {
            const decoded = await commonService.jwtVerify(req.jwtToken);
            if (decoded) {
                const list_Account = await client.query(`select a.account_id,a.company_code,a.company_name,a.city_id,a.state_id,a.mobile_number,a.whatsapp_number,a.reg_number,a.telephone_number1,a.telephone_number2,a.website,a.gstin_number,a.active_status,
                b.city_name,c.state_name,STRING_AGG (e.module_name, ',') module_name, STRING_AGG (cast(e.module_id as text), ',') module_id from tbl_account_master as a inner join tbl_account_module as d on d.account_id=a.account_id left join tbl_def_modules as e on e.module_id=d.module_id 
                inner join  tbl_def_city as b  on b.city_id=a.city_id inner join tbl_def_state as c on c.state_id=a.state_id group by a.account_id,b.city_name,c.state_name ORDER BY a.account_id DESC`);
                if (client) {
                    client.end();
                }
    ...
    try {
        if (req.jwtToken) {
            ...
            if (decoded) {
                ...
                var count_Check = count && count.rows[0].count
                if (count_Check != 0 && count_Check != null && count_Check != undefined && count_Check != "") {
                    const result = await client.query(`UPDATE tbl_account_master SET active_status = $1 where account_id = $2 `, [0, account_id]);
                    if (client) {
                        client.end();
                    }
        ...
        if (req.jwtToken) {
            ...
            if (decoded) {
                ...
                if (max == 0) {
                    ...
                    response_Array = await client.query(`select module_id,module_name from tbl_def_modules where module_id NOT IN (select a.module_id from tbl_account_module as a inner join tbl_account_master as b on a.account_id = b.account_id where a.account_id != $1 and b.active_status = 1 ) and active_status = 1 `, [account_id]);
                }
                if (client) {
                    client.end();
                }
