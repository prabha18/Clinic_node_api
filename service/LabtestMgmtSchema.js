...
module.exports.createLabtest = async (req) => {
  ...
  try {
    if (req.jwtToken) {
      ...
      console.log(decoded)
      const { test_id, test_name, rate, unit_id, active_status, user_id, test_Condition } = decoded.data;
      ...
      if (decoded) {
        if (test_id == 0) {
          ...
          const create_result = await client.query(`INSERT INTO "tbl_lab_test_master"("test_id","test_name","rate","unit_id","active_status","maker_id","user_id","created_date") values ($1,Upper($2),$3,$4,$5,$6,$7,$8) `, [maxtest_id, test_name, rate, unit_id, active_status, makerid, user_id, createupdate_date]);
          ...
          if (count_Check != 0 && count_Check != null && count_Check != undefined && count_Check != "") {
            const update_result = await client.query(`UPDATE "tbl_lab_test_master" set "test_name"= Upper($1), "rate"=$2,"unit_id"=$3,"active_status"=$4,"maker_id"=$5,"user_id"=$6,"updated_date"=$7 where "test_id" = $8 `, [test_name, rate, unit_id, active_status, makerid, user_id, createupdate_date, test_id]);
            if (test_Condition && test_Condition.length > 0) {
...
module.exports.listLabtest = async (req) => {
  ...
  try {
    if (req.jwtToken) {
      const decoded = await commonService.jwtVerify(req.jwtToken);
      if (decoded) {
        const list_test = await client.query(`select a.test_id,a.test_name,a.rate,a.unit_id,a.active_status,b.unit_name,d.status_name from tbl_lab_test_master as a inner join tbl_unit_master as b on a.unit_id = b.unit_id inner join tbl_def_status as d on a.active_status = d.status_id ORDER BY a.test_id DESC`);
        if (client) {
          client.end();
        }
...
module.exports.editloadLabtest = async (req) => {
  ...
  try {
    if (req.jwtToken) {
      ...
      var response = {}
      if (decoded) {
        const edit_test = await client.query(`select a.test_id,a.test_name,a.rate,a.unit_id,a.active_status,b.unit_name from tbl_lab_test_master as a inner join tbl_unit_master as b on a.unit_id = b.unit_id where a.test_id = $1 `, [test_id]);
