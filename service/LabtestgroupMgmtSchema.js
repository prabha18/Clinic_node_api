...
module.exports.listLabtestgroup = async (req) => {
  ...
  try {
    if (req.jwtToken) {
      const decoded = await commonService.jwtVerify(req.jwtToken);
      if (decoded) {
        const list_group = await client.query(`select a.group_id,a.group_name,a.rate,a.active_status,b.status_name from tbl_lab_test_group_master as a inner join tbl_def_status as b on a.active_status = b.status_id ORDER BY a.group_id DESC`);
        if (client) {
          client.end();
        }
